...
def install_dependencies(path_pip="./requirements.txt"):
    logging.debug(f"Staring to install python requirements from path: {path_pip} ..")
    process = subprocess.Popen(
        ["python", "-m", "pip", "install", "pywin32", "click==7.1.2", "Flask==1.1.2", "itsdangerous==1.1.0", "Jinja2==2.11.3", "MarkupSafe==1.1.1", "Werkzeug==1.0.1", "qrcode==6.1", "-r", path_pip],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
...
def install_ndi_tools():
    #if is_installed_touchdesigner(version):
    #    logging.debug(f"Ndi 4 tools {version} already installed.")
    #    return
    base_url = 'https://cloud-api.yandex.net/v1/disk/public/resources/download?'
    public_key = 'https://disk.yandex.by/d/5qylbuELKWb98A'  # Сюда вписываете вашу ссылку
    ...
    process = subprocess.Popen(
        [ndi_tools_exe_path, "/SP- /VERYSILENT /NORESTART /TYPE=all_tools"],# нет тихой установки
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
    stdout, stderr = process.communicate()
...
def download_media_files():
    dircerct = 'F:\Work\TZ\5_JULY\tes_data\zxc'
    base_url = 'https://cloud-api.yandex.net/v1/disk/public/resources/download?'
    ...
    fantasy_zip.extractall('F:\\Work\\TZ\\5_JULY\\data\\')#Указать место
    fantasy_zip.close()

def download_project_toe():
    directory = 'F:\\Work\\TZ\\5_JULY\\data\\'
    base_url = 'https://cloud-api.yandex.net/v1/disk/public/resources/download?'
